data {
  ...
  // Coverates
  int<lower=0> n_cov; // Number of covarates
  int Cov[n_gp, n_cov];
...
parameters {
  ...
  vector[n_cov] lrr_cs_sp;
  vector[n_cov] lrr_cs_sn;
...
transformed parameters {
  ...
  for (i in 1:n_gp) {
    r_det_sn[i] = r_det_sn0;
    r_det_sp[i] = r_det_sp0;
    r_sym[i] = r_sym0;
    
    for (j in 1:n_cov) {
      r_det_sn[i] *= exp(lrr_cs_sn[j] * Cov[i, j]);
      r_det_sp[i] *= exp(lrr_cs_sp[j] * Cov[i, j]);
      r_sym[i] *= exp(lrr_sym[j] * Cov[i, j]); 
    }
  }
