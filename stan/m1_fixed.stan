...
parameters {
  ...
  real<lower=0> r_det_sn; // delay to notification, sm-
  real<lower=0> r_det_sp; // delay to notification, sm+
...
transformed parameters {
  ...
  for (j in 1:n_gp) {
    ...
    sn0[j] = (1 - p_sp) * r_sym / (r_tr + rn[j] + r_det_sn - adr[j]);
    sp0[j] = (p_sp * r_sym + r_tr * sn0[j]) / (rp[j] + r_det_sp - adr[j]);
  ...
  for (i in 1:n_t) {
    for (j in 1:n_gp) {
      ...
      // notification rates
      nr_sn[j, i] = prv[j, i] * pr_sn[j] * r_det_sn;
      nr_sp[j, i] = prv[j, i] * pr_sp[j] * r_det_sp;
...
generated quantities {
  ...
  for (j in 1:n_gp) {
    ...
    dur_sn[j] = 1 / (rn[j] + r_tr + r_det_sn);
    dur_sp[j] = 1 / (rp[j] + r_det_sp);
  }
}
