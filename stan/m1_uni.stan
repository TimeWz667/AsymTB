...
parameters {
  vector<lower=0, upper=1>[n_gp] p_sp; // Probability of sm+ at symptom onset
...
transformed parameters {
  ...
  for (j in 1:n_gp) {
    ...
    rn[j] = r_sc[j] + r_death_bg[j] + r_death_sn[j];
    rp[j] = r_sc[j] + r_death_bg[j] + r_death_sp[j];
    
    sn0[j] = (1 - p_sp[j]) * r_sym[j] / (r_tr[j] + rn[j] + r_det_sn[j] - adr[j]);
    sp0[j] = (p_sp[j] * r_sym[j] + r_tr[j] * sn0[j]) / (rp[j] + r_det_sp[j] - adr[j]);
...
generated quantities {
  ...
  for (j in 1:n_gp) {
    ...
    dur_sn[j] = 1 / (rn[j] + r_tr[j] + r_det_sn[j]);
